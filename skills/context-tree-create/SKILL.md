...
Run `context-tree create --project-path "<project>" --json`. For an additional
tree, pass `--name "<name>" --as "<alias>"` using the requested name and alias. Report whether the
...
Select the created connection by its alias from the returned collection.
If local, delegate to `$context-tree-publish` with the same project path, selected alias, and any
